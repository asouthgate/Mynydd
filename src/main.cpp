...
int main() {
  try {
    ...
    // data, uniform data, etc.
    VkBuffer buffer = createBuffer(context.device, dataSize,
                                   VK_BUFFER_USAGE_STORAGE_BUFFER_BIT);

    // Device memory is different to the buffer in that
    // it is the actual memory allocation that the buffer uses
    // it is where the data is stored
    // but the buffer is the abstraction that we use to access that memory
    ...
    const char *shaderPath =
        "shaders/shader.glsl.spv"; // SPIR-V compiled compute shader
